...
export function SyncToChainButton({ onChainStatus, chain, className }: SyncToChainProps): JSX.Element {
  ...
  const onSyncToChain = useCallback(
          ...
          if (data.passport) {
            ...
            const successSubmit = (
              <p>
                ...
                {attestationProvider?.hasWebViewer && address && (
                  <a
                    href={attestationProvider.viewerUrl(address)}
                    className="underline"
                    target="_blank"
    ...
    [address, attestationProvider, chain.id, loadVerifierContract, readOnChainData, toast]
  );
  ...
  const isActive = attestationProvider?.status === "enabled";
