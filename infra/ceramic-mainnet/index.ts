...
let ceramicCommand = null;
let frontendUrlEcs = null;
let clusterId = null;
const createCluster = false;
...
if (createCluster) {
  ...
  frontendUrlEcs = pulumi.interpolate`http://${httpListener.endpoint.hostname}/`;
  ...
  ceramicCommand = makeCmd(ceramicStateBucketName, pulumi.interpolate`http://${httpListener.endpoint.hostname}:5001`);
  ...
  const httpListener = alb.createListener("gitcoin-ceramic-http", {
    ...
    defaultAction: {
      type: "redirect",
      redirect: {
        host: "ceramic-gitcoin.hirenodes.io",
        protocol: "HTTPS",
        port: "443",
        statusCode: "HTTP_301",
      },
...
export { ceramicCommand };
export { frontendUrlEcs };
export { clusterId };
