...
export function setupIAM() {
  ...
  const passportClusterArn = cluster.arn;
  const passportClusterName = cluster.name;
  ...
  const serviceRecordXyz = new aws.route53.Record("passport-iam-record", {
    name: "iam",
  ...
  const amplifyAppHookUrl = pulumi.secret(amplifyAppInfo.webHook.url);
}
