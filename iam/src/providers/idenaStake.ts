...
import { requestIdentityStake } from "@gitcoin/passport-platforms/dist/commonjs/src/Idena/procedures/idenaSignIn";
...
abstract class IdenaStakeProvider implements Provider {
  ...
  async verify(payload: RequestPayload): Promise<VerifiedPayload> {
    const { valid, address, expiresInSeconds } = await checkStake(payload.proofs.token, this.minStake);
    if (!valid) {
      return { valid: false };
    }
    return {
