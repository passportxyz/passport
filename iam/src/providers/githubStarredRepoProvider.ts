...
// ----- Utils
import { checkUserRepoStars } from "./utils/providerHelpers";
...
export class StarredGithubRepoProvider implements Provider {
  ...
  async verify(payload: RequestPayload): Promise<VerifiedPayload> {
    let valid = false,
      ...
      verifiedUserRepoPayload: VerifiedGithubStarredRepoData = {};
...
const verifyUserGithubRepo = async (
  ...
  code: string
): Promise<VerifiedGithubStarredRepoData> => {
  // retrieve user's auth bearer token to authenticate client
  const accessToken = await requestAccessToken(code);
  ...
  const userRepoStarsCheck: VerifiedGithubStarredRepoData = checkUserRepoStars(userData, repoRequestData);
