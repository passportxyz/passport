...
export class OutdidProvider implements Provider {
  ...
  async verify(payload: RequestPayload): Promise<VerifiedPayload> {
    ...
    try {
      ...
      const verificationData: OutdidVerification = await axios
        .get(`https://api.outdid.io/v1/verification-request?verificationID=${verificationID}`)
      ...
      if (did === undefined || did === "" || did !== payload.proofs?.userDid) {
        ...
        id === undefined ||
        id === "" ||
        JSON.stringify(verificationData.parameters) !== JSON.stringify({ uniqueness: true })
      ) {
