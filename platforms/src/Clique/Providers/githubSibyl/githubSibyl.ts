...
export class GithubSibylProvider implements Provider {
  ...
  async verify(payload: RequestPayload, context: ProviderContext): Promise<VerifiedPayload> {
    console.log("clique github payload:", payload);
...
export const requestAccessToken = async (code: string, context: ProviderContext): Promise<string> => {
  ...
  console.log("clique github requestAccessToken code:", code);
...
export const verifyGithub = async (code: string, context: ProviderContext): Promise<GithubFindMyUserResponse> => {
  try {
    ...
    console.log("clique github verifyGithub result:", JSON.stringify(userRequest.data));

    return userRequest.data as GithubFindMyUserResponse;
  } catch (_error) {
    const error = _error as ProviderError;
    console.log("clique github verifyGithub ERROR:", getErrorString(error));
    return {
      errors: [
